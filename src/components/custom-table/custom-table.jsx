...
// import { Scrollbar } from '../scrollbar';
// import { TableFiltersResult } from './table-filters-result';
// import { CustomTableFiltersResult } from './custom-table-filters-result';
...
//     const statusMatch =
//       normalizedFilterStatus === 'all' || normalizedRowStatus === normalizedFilterStatus;
...
//     (event) => {
//       filters.setState({ ...filters.state, name: event.target.value });
//       setPage(0); // Reset pagination to the first page when searching
//     },
//     [filters]
//   );

//   return (
//     <Card sx={{ boxShadow: '0px 12px 24px -4px rgba(145, 158, 171, 0.2)', width: '100%', mt: 4 }}>
//       {title && (
//         <CardHeader
//           sx={{ p: 3 }}
//           title={
...
//               iconPosition="end"
//               value={tab.value}
//               label={<Tooltip title={tab.tooltip}>{tab.label}</Tooltip>}
//               icon={
...
//   // Add a check for truly empty data
//   const hasNoData = rows.length === 0;

//   // Updated filtering logic to handle both status and search
//   const filteredRows = rows.filter((row) => {
//     const normalizedRowStatus = row.status?.toLowerCase() || '';
//     const normalizedFilterStatus = filters.state.status?.toLowerCase() || 'all';
